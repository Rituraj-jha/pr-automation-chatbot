...
- Before asking for more values, extract any values already present in the user's message, normalize them to canonical config values, and save only accepted collect fields. Use `validate_fields` for uncertain option values or misspellings; if it returns `valid: true`, call `set_fields` with `normalized`.
