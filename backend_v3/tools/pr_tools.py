...
def _derive_labels(resources: list[dict], intake_answers: dict | None = None) -> list[str]:
...
def _create_pr_sync(
    ...
    with httpx.Client(headers=headers, verify=CA_BUNDLE, timeout=30) as client:
        ...
        intake_answers = resources[0].get("intake_answers") if resources else None
        labels = _derive_labels(resources, intake_answers)
